...
class OPVCobsDecoderRandomTest : public ::testing::Test {
 protected:
  ...
  int make_random_packet(uint8_t *buf, uint32_t serial)
  {
    // start the packet with the specified serial number
      * (uint32_t *)buf = serial; // four-byte serial number field
    ...
    for (int i = 4; i < random_length-crc_length; i++)
    {
  ...
  static void check_random_packet(const uint8_t *buf, int len, uint32_t expected_serial)
  {
    std::cerr << "Checking packet " << expected_serial << " of length " << len << std::endl;

    uint32_t received_serial = *(uint32_t *)buf;
  ...
  void send_a_bunch_of_packets(uint32_t target_serial)
  {
      uint32_t serial = 0;
...
{
    const uint32_t target_packet_count = 1000;
